...
All human-readable timing rules must state explicit units. Critical relay timings must state both the human unit and exact seconds, for example `1 minute (60 seconds)`, `10 minutes (600 seconds)`, `11 minutes (660 seconds)`, `12 minutes (720 seconds)`, and `3 minutes (180 seconds)`. Any bare numeric timing delta without an explicit seconds/minutes/hours unit is forbidden. Numeric machine fields ending in `_seconds` are seconds by schema, but adjacent explanatory prose must still state the unit. `RRULE:FREQ=HOURLY` is recurrence metadata and does not convert explicit second offsets into minutes or hours.
