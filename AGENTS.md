...
- Startup: fence -> preserve newest verified continuation -> persist BOOT_STARTED -> arm the same canonical once at approximately wake+11m for the normal ~10m envelope -> verify -> work. Do not restore the retired +13m/+780s horizon, and do not use repeated +2m rolling refresh as the normal strategy.
...
- **Normal path invariant:** normal relay is `MAIN -> SAME MAIN at END+60 -> MAIN`. During the active turn, arm provisional crash insurance once at approximately `WAKE+11m` for the normal ~10m envelope. Normally do not mutate it mid-turn; extend the SAME canonical only if useful work or required close handling clearly threatens to cross that due. Prior live A/B evidence rejects repeated `+2m` rolling refresh as the default because it produced overlap; the former `+13m/+780s` horizon is also retired as unnecessarily long. If a shifted fast wake is lost, the SAME MAIN's natural recurring RRULE occurrence is the cold fallback. Startup Watchdog is disabled break-glass standby and is not required for correctness.
