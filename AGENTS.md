...
- Startup: fence -> preserve newest verified continuation -> short-rolling provisional-arm the same canonical at approximately NOW+120s -> verify -> work. Do not restore the retired long `target work + safety margin` (+780s) provisional.
- Normal `CONTINUE` close: checkpoint -> set THIS SAME canonical's next due to the **actual observed turn END + exactly 60 seconds** (preserve seconds; no minute rounding) -> verify -> persist observed due. **Do not arm or synchronize Watchdog during normal close.** The same MAIN's recurring RRULE is its built-in cold fallback if a shifted wake is lost before bootstrap. A different MAIN delay requires fresh durable evidence/constraint plus an explicit exception reason; scheduler delivery jitter is measured separately.
- **Normal path invariant:** normal relay is `MAIN -> SAME MAIN at END+60 -> MAIN`. During the active turn, provisional MAIN crash insurance is **short rolling**: arm/refresh to approximately `NOW+120s`; when <=45s remains and normal close is not ready, shift the SAME canonical forward to `NOW+120s` and live-verify it. The retired +780s long provisional must not be restored. If a shifted fast wake is lost, the SAME MAIN's natural recurring RRULE occurrence is the cold fallback. Startup Watchdog is disabled break-glass standby and is not required for correctness.
...
- CI pending, packet/substep/checkpoint completion, secured continuation, or `nothing obvious` never authorize a voluntary pre-600s normal `CONTINUE` close.
