...
- **Normal path invariant:** normal relay is `MAIN -> SAME MAIN at END+60 -> MAIN`. The provisional ~13-minute MAIN due is only in-flight crash insurance. The Startup Watchdog is emergency-only. Neither provisional coverage nor Watchdog coverage may be used as the expected normal successor or as justification for normal turn close.
