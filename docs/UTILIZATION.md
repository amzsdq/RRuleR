...
The active performance target is a **rolling mean observed useful-work span >= 840 seconds across at least 3 valid completed turns**. The intended operating shape is approximately 14 minutes of useful work inside each 15-minute wake interval, with roughly one minute of runtime/handoff safety margin.
...
For the current experiment, the acceptance metric is deliberately stricter and easier to reconstruct from durable evidence:

`observed_useful_span_seconds = last_meaningful_progress_at - substantive_work_started_at`

This is a span metric, not a claim that every second between the boundaries was CPU-active. It is valid only when both boundaries were durably recorded during the same authority-owned run.
...
5. **Progress evidence** — after meaningful bounded units, refresh `last_progress_at` and describe the current unit. When practical, keep durable progress gaps <= 120 seconds during the measurement experiment.
...
The utilization root cannot pass merely because the no-self-termination policy exists. It requires live successor-cycle evidence that run termination is classified with the measurable end contract and that any gap/overlap is reported from durable timestamps.

PASS requires at least 3 valid completed turns and a rolling mean of their last three `observed_useful_span_seconds` values >= 840. Until then, utilization handoff measurement remains blocking and program status remains `CONTINUE`.
